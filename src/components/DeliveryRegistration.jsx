...
function DeliveryRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (stage === 1) {
      try {
        const response = await fetch(`https://local-treasures.onrender.com/delivery-agent/register`, {
          method: 'POST',
  ...
  const verifyOtp = async () => {
    try {
      const response = await fetch(`https://local-treasures.onrender.com/delivery-agent/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
